...
public class GasStation {
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        int current_gas = 0;
        int current_cost_gas = 0;
        int initial_position = 0;

        // assuming the size of gas and cost would always be the same
        for (int i = 0; i < gas.length; i++) {

            current_gas += gas[i] - cost[i];
            current_cost_gas += gas[i] - cost[i];

            if (current_cost_gas<0){
        ...
        return current_gas>=0? initial_position:-1;
    ...
    public static void main(String[] args) {
        ...
        int [] gas = {1,2,3,4,5};
        int[] cost = {3,4,5,1,2};
        System.out.println("Gas Station: " + gs.canCompleteCircuit(gas,cost));
    }
}